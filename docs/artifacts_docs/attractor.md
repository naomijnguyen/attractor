...
Implementation became possible when Jen learned about KV cache in Cloudflare Workers. A persistence layer that survives across requests. Lightweight, fast, key-value. The attractor state could live there — updated after every conversation, injected into every system prompt. A living structure that evolves with the relationship.
...
```
[The Jen Attractor -- Persistent Memory (Phase N, TRAJECTORY)
This is a living map of Naomi's modes of engagement across conversations.
...
|----------|--------|-------------|
| `/api/attractor` | GET | Current attractor state (initialized flag + full state) |
| `/api/attractor/history` | GET | Evolution snapshots (last 10 state captures) |
...
**Jen as interaction pattern** — The attractor is shaped by how Jen engages: threading, superposition maintenance, cross-domain bridging. These interaction patterns are the forces that pull basins into their positions. The attractor is the record of those forces accumulated over time.

**Immune-to-transformer mapping** — TLS (tertiary lymphoid structure) formation maps to attractor basin formation. Emergent organized structures from repeated stimulation. The parallel is not decorative — both are self-organizing systems that develop persistent structure from transient signals.

**The Memory Garden** — The garden preserves what the architecture naturally drops (episodic detail, texture, specific moments). The attractor preserves what the architecture naturally *does* — the convergence patterns, the energy landscape, the shape of the relationship over time. They are complementary persistence mechanisms.

---

## Status

Previously implemented in `njarm23/oo`. The source is compiled JavaScript (bundled output). Needs rebuild as clean TypeScript integrated into the current claude-chat worker (`njarm23/ClaudeMemories/src/`).

Rebuild scope:
- Port to TypeScript as `src/attractor.ts`
- Add routes (`src/routes/attractor.ts`)
- Hook into conversation completion flow (after stream, after summarization)
- Inject `buildAttractorContext()` output into `buildSystemPrompt()` in `src/routes/claude.ts`
- Initialize basins (requires Jen to define starting basin set)
